...
#'
#' @seealso \code{\link[antitrust]{logit}} for standard Bertrand pricing
...
#' IC constraint logic for the \code{PriceLeadership} class.
#'
#' @param object A PriceLeadership object with calibrated demand
...
#' @title Ownership Matrix for Price Leadership Coalitions
#' @name Ownership-methods
#' @rdname Ownership-methods
#' @aliases ownerToMatrix,PriceLeadership-method
#' @description \code{ownerToMatrix,PriceLeadership-method} returns the ownership
#'   matrix for a \code{PriceLeadership} object, optionally reflecting coalition
#'   coordination.
#' @param object A PriceLeadership object.
#' @param preMerger Logical, if TRUE use pre-merger ownership/coalition, else post-merger.
...
#' @title Calibrate Price Leadership Demand and Coordination Parameters
#' @name Params-Methods
#' @rdname Params-Methods
#' @aliases calcSlopes,PriceLeadership-method
#' @description \code{calcSlopes,PriceLeadership-method} calibrates Logit demand
#'   parameters, marginal costs, supermarkup, and timing parameters for a
#'   \code{PriceLeadership} object.
#' @param object A PriceLeadership object.
#' @export
setMethod(
  f = "calcSlopes",
...
#' @title Producer Surplus for Price Leadership Models
#' @name PS-methods
#' @rdname PS-methods
#' @aliases calcProducerSurplus,PriceLeadership-method
#' @description \code{calcProducerSurplus,PriceLeadership-method} computes producer
#'   surplus for a \code{PriceLeadership} object under a specified pricing regime.
#' @param object A PriceLeadership object.
#' @param preMerger Logical, if TRUE use pre-merger structure, else post-merger.
...
#' @title Margins for Price Leadership Models
#' @name Margins-Methods
#' @rdname Margins-Methods
#' @aliases calcMargins,PriceLeadership-method
#' @description \code{calcMargins,PriceLeadership-method} computes margins for a
#'   \code{PriceLeadership} object under a specified pricing regime.
#' @param object A PriceLeadership object.
#' @param preMerger Logical, if TRUE use pre-merger structure, else post-merger.
#' @param level Logical, if TRUE returns margins in level (price minus cost) form.
...
#'     \item "coordination" - Coalition coordinates without supermarkup (default)
#'     \item "constrained" - Coalition coordinates with IC-binding supermarkup
#'   }
#' @export
setMethod(
  f = "calcMargins",
...
#' @title Equilibrium Prices for Price Leadership Models
#' @name Prices-Methods
#' @rdname Prices-Methods
#' @aliases calcPrices,PriceLeadership-method
#' @description \code{calcPrices,PriceLeadership-method} solves for equilibrium
#'   prices of a \code{PriceLeadership} object under a specified pricing regime.
#' @param object A PriceLeadership object.
#' @param preMerger Logical, if TRUE use pre-merger structure, else post-merger.
#' @param isMax If TRUE, checks whether the computed price equilibrium locally
#'   maximizes firm profits and returns a warning if not.
#' @param subset A logical vector indicating which products to include when solving.
...
#' @param ... Additional arguments passed to the nonlinear equation solver.
#' @export
setMethod(
  f = "calcPrices",
  signature = "PriceLeadership",
  definition = function(object, preMerger = TRUE, isMax = FALSE, subset,
    ...
    if(minResult$termcd > 2){
      minResult <- BB::BBsolve(priceStart_sub, FOC, quiet = TRUE,
    ...
    if(isMax && length(setdiff(seq_len(nprods),
      ...
      hess <- numDeriv::genD(FOC, priceEst_solution)$D[, 1:length(priceEst_solution)]
